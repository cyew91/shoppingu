...
function ($scope, $stateParams, $state, $timeout, GetTravelByProfileId, GetTravelByTravelId, $window, GetTravelProductByTravelId) {
    ...
    $scope.deletePost = function (index) {
        ...
        // CancelTravelBycancelTravelId.
        if (!postTravel.updated) {
            postTravel.updated = [];
        }

        postTravel.updated.push(new Date().getTime());

        GetTravelByTravelId.update({
            getAllPostTravelByTravelId: postTravel.postTravelId
        }, function (result) {
            $scope.msg = result;
        });
