...
exports.signout = function(req, res) {
  console.log(
    "Logout: { profileId: " + req.ProfileID + ", loginId: " + req.LoginID + " }"
  );
  req.logout();
...
exports.create = function(req, res, next) {
  ...
  console.log(
    "New User (local) : { id: " + user.id + " username: " + user.username + " }"
  );
...
function authenticateSocialUser(profile, done) {
  ...
  db.User.find({ where: searchQuery })
    .then(function(user) {
      if (!user) {
        var userObj = {};
        if (profile.name) {
...
exports.twitterSocialUser = function(req, res) {
  ...
  function obtainRequestToken() {
    ...
    request.post({ url: requestTokenUrl, oauth: requestTokenOauth }, function(
      err,
      response,
      body
    ) {
...
exports.googleSocailUser = function(req, res) {
  ...
  function getAccessToken(err, response, token) {
    ...
    request.get(
      { url: peopleApiUrl, headers: headers, json: true },
      retrivedInfo
    );
  }
